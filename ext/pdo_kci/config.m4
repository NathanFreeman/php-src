...
if test "$PHP_PDO_OCI" != "no"; then
  ...
  case $PDO_KCI_VERSION in
    7.3|8.0|8.1)
      AC_MSG_ERROR([Oracle client libraries < 9 are not supported])
      ;;
  esac

  PHP_ADD_LIBRARY(clntsh, 1, PDO_KCI_SHARED_LIBADD)
  PHP_ADD_LIBPATH($PDO_KCI_LIB_DIR, PDO_KCI_SHARED_LIBADD)

  PHP_CHECK_LIBRARY(clntsh, OCIEnvCreate,
  [
    AC_DEFINE(HAVE_DCIENVCREATE,1,[ ])
  ], [], [
    -L$PDO_KCI_LIB_DIR $PDO_KCI_SHARED_LIBADD
  ])

  PHP_CHECK_LIBRARY(clntsh, DCIEnvNlsCreate,
  [
    AC_DEFINE(HAVE_DCIENVNLSCREATE,1,[ ])
  ], [], [
    -L$PDO_KCI_LIB_DIR $PDO_KCI_SHARED_LIBADD
  ])

  dnl Scrollable cursors?
  PHP_CHECK_LIBRARY(clntsh, DCIStmtFetch2,
  [
     AC_DEFINE(HAVE_DCISTMTFETCH2,1,[ ])
  ], [], [
    -L$PDO_KCI_LIB_DIR $PDO_KCI_SHARED_LIBADD
  ])
  ...
  PHP_NEW_EXTENSION(pdo_kci, pdo_kci.c kci_driver.c kci_statement.c, $ext_shared,,-I$pdo_cv_inc_path)

  PHP_SUBST_OLD(PDO_KCI_SHARED_LIBADD)
  PHP_SUBST_OLD(PDO_KCI_DIR)
  PHP_SUBST_OLD(PDO_KCI_VERSION)

  PHP_ADD_EXTENSION_DEP(pdo_kci, pdo)

  AC_DEFINE_UNQUOTED(PHP_PDO_KCI_CLIENT_VERSION, "$PDO_KCI_VERSION", [ ])
fi
